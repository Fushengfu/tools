...
class WechatApp
{
  ...
  /**
	 *  登录凭证校验 code2Session
	 *  @param string $code 消息数组
	 *  @param bool $append 是否在原消息数组追加
	 */
  public function code2Session($code)
  {
  	$url = self::BASE_URL.self::JSCODE_2_SESSION.'appid='.$this->appid.'&secret='.$this->appsecret.'&js_code='.$code.'&grant_type=authorization_code';
  	return http_get($url);
  }
	...
	public function getUserInfo($access_token,$openid, $lang = 'zh_CN'){
		...
		if ($result)
		{
			...
			if (isset($json['errcode'])) {
				...
				p($errMsgInfo,false,storage_path('framework/cache/').'wechat_error'.date('Ymd').'.txt');
				return false;
			}
			return $json;
		}
